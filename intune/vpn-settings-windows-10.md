...
- **Server**: Fügen Sie mindestens einen VPN-Server hinzu, mit dem Geräte eine Verbindung herstellen. Wenn Sie einen Server hinzufügen, geben Sie folgende Informationen ein:
  ...
  - **IP-Adresse oder FQDN**: Geben Sie die IP-Adresse oder den vollqualifizierten Domänennamen (FQDN, Fully Qualified Domain Name) des VPN-Servers ein, mit dem Geräte eine Verbindung herstellen, z.B. **192.168.1.1** oder **vpn.contoso.com**.
...
- **Verbindungstyp**: Wählen Sie den VPN-Verbindungstyp aus der folgenden Liste von Anbietern aus:
  ...
  Wenn Sie einen VPN-Verbindungstyp auswählen, werden Sie möglicherweise aufgefordert, folgende Einstellungen vorzunehmen:  
    - **Immer aktiviert**: **Aktivieren** Sie diese Option, um die VPN-Verbindung automatisch bei folgenden Ereignissen herzustellen: 
...
- **Benennen Sie die Richtlinie zur Tabelle (NRPT) Regeln**: Namensauflösungsrichtlinie-Tabelle (NRPT)-Regeln zu definieren, wie Namen, wenn mit dem VPN verbunden von DNS aufgelöst wird. Wenn eine VPN-Verbindung hergestellt wurde, können Sie festlegen, welche DNS-Server von der VPN-Verbindung verwendet werden sollen.
...
**DNS-Suffixe für vertrauenswürdige Netzwerke**: Wenn der Benutzer bereits eine Verbindung mit einem vertrauenswürdigen Netzwerk hergestellt hat, können Sie verhindern, dass Geräte automatisch eine Verbindung mit anderen VPN-Verbindungen herstellen.
