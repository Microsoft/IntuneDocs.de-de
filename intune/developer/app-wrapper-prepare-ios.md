...
|**-ac**|(Optional) `<Client ID of the input app if the app uses the Azure Active Directory Authentication Library>` Dies ist die GUID im Feld "Client-ID" aus der Liste Ihrer APP auf dem Blatt "App-Registrierung". |
|**-ar**|(Optional) `<Redirect/Reply URI of the input app if the app uses the Azure Active Directory Authentication Library>` Dies ist der Umleitungs-URI, der in Ihrer APP-Registrierung konfiguriert ist. In der Regel ist dies das URL-Protokoll der Anwendung, zu der die Microsoft Authenticator-App nach der Broker Authentifizierung zurückkehren würde. |
