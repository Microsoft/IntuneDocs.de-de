...
|**-aa**|(Optional) `<Authority URI of the input app if the app uses the Azure Active Directory Authentication Library>`, d. h. `login.windows.net/common` |
|**-ac**|(Optional) `<Client ID of the input app if the app uses the Azure Active Directory Authentication Library>` ist dies die GUID im Feld "Client-ID" aus der Liste Ihrer APP auf dem Blatt für die APP-Registrierung. |
|**-ar**|(Optional) `<Redirect/Reply URI of the input app if the app uses the Azure Active Directory Authentication Library>` ist dies der Umleitungs-URI, der in Ihrer APP-Registrierung konfiguriert ist. In der Regel ist dies das URL-Protokoll der Anwendung, zu der die Microsoft Authenticator-App nach der Broker Authentifizierung zurückkehren würde. |
...
|**-citrix**|Optionale Schließen Sie das Citrix xenmobile App SDK ein (nur Netzwerk Variant). Sie müssen das [Citrix MDX Toolkit](https://docs.citrix.com/en-us/mdx-toolkit/about-mdx-toolkit.html) installiert haben, um diese Option verwenden zu können. |
...
| Verhindern des Standardbuildupdates |Boolen|false| Entspricht der Verwendung von „-b“ ohne Argumente|
| Build String Override |Zeichenfolge|empty| Die benutzerdefinierte CFBundleVersion der umschlossenen Ausgabe-App|
| Einbinden des Citrix xenmobile App SDK (nur Netzwerk Variant)|Boolesch|false| Identisch mit-Citrix|
